...
class ShoppingController extends Controller{
	...
	function ajax_shopping_op($data){
		...
		if($oid > 0 && $sid>0 && !empty($sn)){
			$row = $this->App->findrow("SELECT * FROM `{$this->App->prefix()}goods_order_info` WHERE order_id='$oid' LIMIT 1");
			$ssn = $row['sn_id'];
			if(!empty($ssn)&&$ssn!==$sn){
			...
			if($this->App->update('goods_order_info',array('sn_id'=>$sn,'shipping_id_true'=>$sid),'order_id',$oid)){
				...
				$shopping_name = $this->App->findvar("SELECT shipping_name FROM `{$this->App->prefix()}shipping_name` WHERE shipping_id='{$shopping_id}' LIMIT 1");
				  file_put_contents('./log.l',serialize(array('tel'=>$row['mobile'],'express'=>$shopping_name,'number'=>$sn,'type'=>'tmp_goods')));
